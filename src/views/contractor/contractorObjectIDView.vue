   ...
   <div style="display: flex;">
      ...
      <div v-if="disabled" class="calc">
         <AppTable v-if="search.tableRows.length" :tableHeadline="search.tableHeadline" :tableRows="search.tableRows"
            ...
            <template #check="{ item }">
               <el-checkbox @change="addToMass(item, $event)" size="large" />
            </template>
         </AppTable>
      </div>
   </div>
...
export default {
   ...
   methods: {
      addToMass(val, event) {
         if (event == true) {
            this.activeCheckBox.push(val)
         } else {
            this.activeCheckBox = this.activeCheckBox.filter(item => item !== val)
         }

         if (this.activeCheckBox.length === this.search.tableRows.length) {
            this.disabledNextBtn.check = false
         }
      },
      async fetchLogs() {
